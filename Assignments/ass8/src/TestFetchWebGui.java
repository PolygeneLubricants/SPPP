...
public class TestFetchWebGui {
  ...
  private static void goodFetch() {
    ...
    DownloadWorkerContainer downloadTasks = new DownloadWorkerContainer(textArea, urls, progressBar);
    fetchButton.addActionListener(new ActionListener() {
        public void actionPerformed(ActionEvent e) {
  ...
  public static class DownloadWorkerContainer {
      private DownloadWorker[] workers;
      private AtomicInteger progress;

      public DownloadWorkerContainer(TextArea textArea, String[] urls, JProgressBar progressBar) {
          progress = new AtomicInteger();
          workers = new DownloadWorker[urls.length];
          progressBar.setMaximum(workers.length);
          for(int i = 0; i < workers.length; i++) {
              workers[i] = new DownloadWorker(textArea, urls[i]);
